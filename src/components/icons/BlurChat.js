import React from 'react';

const BlurChat = () => {
    return (
        <div>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M15 10C15 13.3137 12.3137 16 9 16C7.81929 16 6.71824 15.659 5.78998 15.07L3 16L3.92999 13.21C3.34104 12.2818 3 11.1807 3 10C3 6.68629 5.68629 4 9 4C12.3137 4 15 6.68629 15 10Z" stroke="#D9D9D9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M9.33789 15.9896C10.1586 18.3254 12.3838 20 15.0001 20C16.1808 20 17.2819 19.659 18.2101 19.07L21.0001 20L20.0701 17.21L20.2122 16.9744C20.7136 16.0977 21.0001 15.0823 21.0001 14C21.0001 10.6863 18.3138 8 15.0001 8L14.7752 8.00414L14.6623 8.01014" stroke="#D9D9D9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
        </div>
    );
};

export default BlurChat;
