...
const Popup = ({ savePage, getPageList }) => {
    ...
    useEffect(() => {
        ...
        getPageList(accessToken, state).then(data => {
    ...
    const errorNotification = (placement, value, error) => {
        api.info({
            description: <div><NotificationHeader>{`${value}`}</NotificationHeader><Notification>{`${error}`}</Notification></div>,
            placement,
            style: {
                width: 400,
    ...
    return (
        <Modal
            ...
            footer={null}>
            <div>
                <ModalContentBox>Facebook List</ModalContentBox>
                <Modal_body>
                    {contextHolder}
                    {
                        pageList?.page_list?.length > 0 ?
                            pageList?.page_list?.map((data, index) => (
                                <CardListBox key={index} >
                                    ...
                                    {unLock == data?.id && <div className="button"> <ConfirmButton onClick={() => confirm(data)}>Confirm</ConfirmButton></div>}
                                </CardListBox>
                            ))
                            :
                            <ErrorContent>
                                ...
                                <Button
                                    ...
                                    onClick={() => window.open("https://www.facebook.com/pages/creation?ref_type=launch_point", "_blank")}
                                >
                                    Create
                                </Button>
                                <Button
